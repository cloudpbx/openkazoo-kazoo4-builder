...
make publish                      # assembles the apt repo under build/repo/
make test                         # bats unit tests
make help                         # list all targets
```

Each component builds natively for the host architecture inside the Debian 12
build image (`docker/Dockerfile.debian-12`).

## How it fits together

- `config/` — version pins (single source of truth).
- `docker/Dockerfile.debian-12` — build image (kerl, rebar3, full toolchain).
