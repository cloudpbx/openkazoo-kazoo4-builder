...
# --- fs_cli — the ESL CLI client operators use to control/debug a running
# FreeSWITCH (fs_cli -x "sofia status" / reloadxml / show channels|calls). The
# top-level `make` does NOT build it. Compile it directly here rather than via
# the generated libs/esl Makefile: that path links libedit for line-editing,
# which would add a libedit2 runtime dependency the base image does not carry
# (the freeswitch deb declares no Depends and relies on the base image for
# runtime libs). Building with -DDISABLE_CC drops editline, so fs_cli needs only
# libncurses/pthread/libc — all already present because FS core links them. The
# one-shot (-x) and basic interactive console both work; only fancy line-edit
# history is lost. esl_config_auto.h was generated by ./configure above.
_ESL="$B/freeswitch/libs/esl"
gcc -O2 -o "$_ESL/fs_cli" -I"$_ESL/src/include" -DDISABLE_CC \
  "$_ESL/fs_cli.c" \
  "$_ESL"/src/esl.c "$_ESL"/src/esl_event.c "$_ESL"/src/esl_threadmutex.c \
  "$_ESL"/src/esl_config.c "$_ESL"/src/esl_json.c "$_ESL"/src/esl_buffer.c \
  "$_ESL"/src/cJSON.c "$_ESL"/src/cJSON_Utils.c \
  -lncurses -lpthread -lm
[ -x "$_ESL/fs_cli" ] || die "fs_cli not built — check libs/esl sources"
...
# fs_cli (built above) + a sample config. The .deb must NOT hardcode an ESL
# password (it is randomized per deployment and stored in SSM); kazoo-deploy
# writes /etc/fs_cli.conf with the real password. Ship the upstream sample as
# .conf.sample so the format is discoverable on the box.
cp -a "$_ESL/fs_cli" "$STAGE/usr/bin/"
[ -f "$STAGE/usr/bin/fs_cli" ] || die "fs_cli not staged into the deb"
mkdir -p "$STAGE/etc"
[ -f "$_ESL/fs_cli.conf" ] && cp -a "$_ESL/fs_cli.conf" "$STAGE/etc/fs_cli.conf.sample"
